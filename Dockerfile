...
# Use the node influencer from the image (instead of the root influencer)
USER node
...
COPY --chown=node:node --from=development /app/influencer/node_modules ./node_modules
...
COPY --chown=node:node --from=build /app/influencer/node_modules ./node_modules
COPY --chown=node:node --from=build /app/influencer/dist ./dist
